#!/bin/bash
# Run only Stage 2 from pipeline.sh: proxy student gradient computation.
# Usage Example: EXP_DIR=./experiments HF_HUB_OFFLINE=1 HF_DATASETS_OFFLINE=1 TRANSFORMERS_OFFLINE=1 ./pipeline_stage2.sh
...
if [ ! -e "${grad_sentinel}" ] && [ ! -e "${holdout_config}" ]; then
    echo "Missing ${holdout_config}. Run pipeline_stage1.sh first, or set EXP_DIR to an experiment with holdout traces." >&2
    exit 1
fi
...
duration=$SECONDS
printf "${WHITE}\nStage 2 completed in %02dh:%02dm:%02ds${RESET}\n" \
